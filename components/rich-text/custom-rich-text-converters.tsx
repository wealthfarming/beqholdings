...
export function getCustomRichTextConverters(defaultConverters: any) {
  return {
    ...
    heading: ({ node, nodesToJSX, converters, parent, childIndex }: any) => {
      ...
      if (node.tag === 'h1') {
        return <h1 className="text-3xl md:text-4xl font-bold text-[#FFFFFF] mt-8 mb-4">{children}</h1>;
      }
      if (node.tag === 'h2') {
        return <h2 className="text-2xl md:text-3xl font-semibold text-[#FFFFFF] mt-6 mb-3">{children}</h2>;
      }
      if (node.tag === 'h3') {
        return <h3 className="text-3xl md:text-4xl font-bold text-[#FFFFFF] mt-4 mb-2">{children}</h3>;
      }
      if (node.tag === 'h4') {
    ...
    upload: ({ node }: any) => {
      ...
      if (!value || !value.url) {
        return (
          <div className="my-6 bg-gray-100 rounded-lg p-4 text-center text-[var(--muted-foreground)]">
            Image not available
          </div>
        );
      }
